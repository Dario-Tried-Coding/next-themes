...
type Mode = { type: 'mode'; selector?: CssSelector | CssSelector[]; store?: boolean }

export type ModeMono<V extends string = string> = Mode & { strategy: Strats['mono']; key: V; colorScheme: ResolvedMode }

export type ModeMulti<V extends string[] = string[]> = Mode & { strategy: Strats['multi']; keys: { [K in V[number]]: ResolvedMode }; preferred: V[number] }
